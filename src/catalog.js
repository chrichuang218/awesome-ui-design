...
export const projectTypes = [
  { id: 'product', label: '产品界面', description: '产品 / 桌面工具' },
  { id: 'marketing', label: '落地页', description: '品牌 / 落地页' },
  { id: 'portfolio', label: '作品集', description: '博客 / 作品集' }
];
export const usesProjectType = intent => intent === 'inspire' || intent === 'create';
const routes = {
...
export function getRoute(intent, project = 'product') {
  const base = routes[intent] || routes.inspire;
  const route = { ...base, ids: [...base.ids], reasons: [...base.reasons], steps: [...base.steps] };
  if (intent === 'create' && ['marketing', 'portfolio'].includes(project)) {
    route.ids[1] = 'r53'; route.steps[1] = '生成页面原型'; route.reasons[1] = '带上真实内容和参考，让 frontend-design 建立明确的视觉方向。';
    route.ids[0] = project === 'marketing' ? 'r3' : 'r4';
    route.steps[0] = project === 'marketing' ? '参考落地页结构' : '参考作品呈现';
    route.reasons[0] = project === 'marketing' ? '先看同类页面如何介绍产品、说明价值并引导下一步操作。' : '观察作品集如何组织案例、介绍创作过程与突出代表作品。';
    route.title = project === 'marketing' ? '梳理产品价值，再做落地页。' : '围绕代表作品，搭建你的作品集。';
    route.description = project === 'marketing' ? '从页面结构参考开始，带上真实产品内容生成第一版，再检查移动端与主操作。' : '先选案例展示方式，再把自己的作品与介绍放进页面，最后检查阅读和导航体验。';
    route.reasons[1] = project === 'marketing' ? '提供目标用户、产品卖点和主操作，让 frontend-design 生成有明确重点的页面。' : '提供代表作品、案例说明和风格参考，让 frontend-design 生成适合个人表达的页面。';
  } else if (intent === 'create') {
    route.title = '先定界面规则，再做产品原型。';
  }
  if (intent === 'inspire' && ['marketing', 'portfolio'].includes(project)) {
    route.ids[0] = project === 'marketing' ? 'r3' : 'r4'; route.steps[0] = '观察页面结构'; route.reasons[0] = '选择与你的页面目的接近的案例，观察内容顺序和主操作。';
    route.title = project === 'marketing' ? '先看落地页，再确定视觉方向。' : '从作品集案例，找到自己的表达。';
    route.description = project === 'marketing' ? '先看信息如何展开，再选一个视觉参考，最后观察按钮与导航的细节。' : '先看作品如何呈现，再明确整体风格，最后学习案例浏览与导航的细节。';
    route.reasons[0] = project === 'marketing' ? '按页面区块查看落地页案例，比较产品介绍、功能说明与行动入口的安排。' : '从创意网站与作品集案例中，观察项目展示、个人介绍和页面叙事。';
  }
  return route;
}
